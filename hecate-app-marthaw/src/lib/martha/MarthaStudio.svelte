<script lang="ts">
	...
	// --- Cross-cutting agent components ---
	import CostBudgetBar from './cost_budget/CostBudgetBar.svelte';
	import KnowledgeGraphPanel from './knowledge_graph/KnowledgeGraphPanel.svelte';
	import VentureNarrative from './venture_narrative/VentureNarrative.svelte';
	...
	type WorkspaceTab = 'gates' | 'agents' | 'knowledge' | 'narrative';

	function activateTab(tab: WorkspaceTab) {
		const wasActive =
			tab === 'gates' ? showGateInbox :
			tab === 'agents' ? showAgentPipeline :
			tab === 'knowledge' ? showKnowledgeGraph :
			showNarrative;
		showGateInbox = false;
		...
		showNarrative = false;
		if (!wasActive) {
			if (tab === 'gates') showGateInbox = true;
			else if (tab === 'agents') showAgentPipeline = true;
			else if (tab === 'knowledge') showKnowledgeGraph = true;
			else if (tab === 'narrative') showNarrative = true;
		}
	}
...
<div class="flex flex-col h-full">
	...
	{:else}
		...
		<!-- Workspace tab bar + status indicators -->
		<div class="border-b border-surface-700/50 bg-surface-800/30 px-4 py-0 shrink-0 flex items-center gap-0">
			<!-- Gates tab -->
			<button
				onclick={() => activateTab('gates')}
				class="relative px-3 py-2 text-[11px] font-medium transition-colors border-b-2 -mb-px
					{showGateInbox
					? 'text-hecate-300 border-hecate-400'
					: 'text-surface-500 border-transparent hover:text-surface-300 hover:border-surface-500'}"
			>
				Gates
				{#if $hasPendingGates}
					<span class="ml-1 px-1.5 py-0.5 rounded-full text-[8px] font-bold bg-amber-500/20 text-amber-300 animate-pulse">
						{$pendingGates.length}
					</span>
				{/if}
			</button>
			...
			<button
				...
				class="px-3 py-2 text-[11px] font-medium transition-colors border-b-2 -mb-px
					{showAgentPipeline
					? 'text-hecate-300 border-hecate-400'
					: 'text-surface-500 border-transparent hover:text-surface-300 hover:border-surface-500'}"
			>
			...
			<!-- Budget (inline, compact — only shown when budget is set) -->
			<CostBudgetBar />

			<!-- Connection status -->
			<span class="flex items-center gap-1.5 ml-3 text-[10px]">
				<span
					class="inline-block w-1.5 h-1.5 rounded-full {connectionStatus === 'connected'
						? 'bg-success-400'
						: connectionStatus === 'connecting'
		...
		<div class="flex flex-1 overflow-hidden relative">
			...
			{#if $divisions.length > 0 || $pendingGates.length > 0}
				<NerveCenter
					onSelectAgent={(role) => { clearPanels(); showAgentPipeline = true; }}
					onSelectGate={(sessionId) => { clearPanels(); showGateInbox = true; }}
				/>
			{/if}
			...
			<div class="flex-1 flex flex-col overflow-hidden">
				...
				<div class="flex-1 flex flex-col overflow-hidden">
					...
					{:else}
						<VentureOverview
							onSelectDivision={(divId) => { clearPanels(); }}
						/>
					{/if}
				</div>
