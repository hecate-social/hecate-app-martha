<script lang="ts">
	...
	import { aiModel, setAIModelOverride, autoSpeak, isSpeaking } from '$lib/martha/shared/aiStore.js';
	import { systemPrompt } from '$lib/martha/shared/personality.js';
	import { visionOraclePrompt, applyVariables } from '$lib/martha/shared/agentsStore.js';
	import { createStudioContext } from '$lib/martha/shared/context.js';
	import type { ChatStream } from '$lib/martha/shared/context.js';
	import ModelSelector from '$lib/martha/shared/ModelSelector.svelte';
	import VoiceControls from '$lib/martha/shared/VoiceControls.svelte';
	import type { ChatMessage, StreamChunk } from '$lib/martha/types.js';
	...
	// Match ```markdown, ```md, or bare ``` containing vision-like content (## heading or **bold**)
	const VISION_HEADING = /^#{1,3}\s|^\*\*/m;

	function stripVisionFences(content: string): string {
		let result = content.replace(
			/```(?:markdown|md)?\n([\s\S]*?)```/g,
			(_match, inner) => VISION_HEADING.test(inner) ? '\u{25C7} Vision updated \u{2197}' : _match
		);
		result = result.replace(
			/```(?:markdown|md)?\n([\s\S]*)$/,
			(_match, inner) => VISION_HEADING.test(inner) ? '\u{25C7} Synthesizing vision... \u{2197}' : _match
		);
		return result;
	}
	...
	// Extract vision markdown from content — tries fenced blocks first, then raw markdown with headings
	function extractVisionFromContent(content: string): string | null {
		// Try fenced code blocks: ```markdown, ```md, or bare ``` with vision content
		const fencePattern = /```(?:markdown|md)?\n([\s\S]*?)```/g;
		let best: string | null = null;
		let m;
		while ((m = fencePattern.exec(content)) !== null) {
			if (VISION_HEADING.test(m[1])) best = m[1].trim();
		}
		if (best) return best;

		// No fenced vision found — look for raw markdown with vision-like structure
		// (multiple headings in the visible content, outside of think blocks)
		const withoutThink = content.replace(/<think>[\s\S]*?<\/think>/g, '');
		const headingCount = (withoutThink.match(/^#{1,3}\s+.+/gm) || []).length;
		if (headingCount >= 3) {
			const firstHeading = withoutThink.search(/^#{1,3}\s+.+/m);
			if (firstHeading >= 0) return withoutThink.slice(firstHeading).trim();
		}
	...
	let extractedVision = $derived.by(() => {
		for (let i = chatMessages.length - 1; i >= 0; i--) {
			if (chatMessages[i].role === 'assistant') {
				const vision = extractVisionFromContent(chatMessages[i].content);
				if (vision) return vision;
			}
		}
		// Also check streaming content — complete or partial fence
		if (streamingContent) {
			const complete = extractVisionFromContent(streamingContent);
			if (complete) return complete;
			// Try partial fence (still streaming)
			const partialMatch = streamingContent.match(/```(?:markdown|md)?\n([\s\S]*)$/);
			if (partialMatch && VISION_HEADING.test(partialMatch[1])) return partialMatch[1].trim();
		}
		return null;
	});
	...
	async function sendMessage(content: string) {
		...
		stream
			...
			.onDone(async (chunk: StreamChunk) => {
				...
				// Auto-speak if enabled (strip vision fences for cleaner speech)
				if (get(autoSpeak) && accumulated) {
					const spokenText = accumulated.replace(/```markdown\n[\s\S]*?```/g, '').trim();
					if (spokenText) speakMessage(spokenText);
				}
			})
	...
	function handleKeydown(e: KeyboardEvent) {
		...
		} else if (e.key === 'Escape' && isStreaming && activeStream) {
			e.preventDefault();
			activeStream.cancel();
			activeStream = null;
			isStreaming = false;
	...
	<div class="flex flex-col overflow-hidden" style="width: {splitPercent}%">
		...
		<div class="flex items-center gap-2 px-4 py-2.5 border-b border-surface-600 shrink-0">
			...
			<div class="relative">
				<button
					...
					class="text-[9px] px-1 rounded transition-colors
						{showShortcuts ? 'text-hecate-400' : 'text-surface-500 hover:text-surface-300'}"
				...
				{#if showShortcuts}
					<!-- svelte-ignore a11y_no_static_element_interactions -->
					<div class="fixed inset-0 z-40" onclick={() => (showShortcuts = false)}></div>
					<div class="absolute top-full right-0 mt-1 z-50 w-52 bg-surface-700 border border-surface-500 rounded-lg shadow-xl overflow-hidden">
						<div class="px-3 py-1.5 text-[9px] font-semibold text-surface-400 uppercase border-b border-surface-600">
							Keyboard Shortcuts
						</div>
						<div class="px-3 py-2 space-y-1.5 text-[10px]">
							<div class="flex justify-between">
								<span class="text-surface-300">Push-to-talk</span>
								<span class="text-surface-400"><kbd class="px-1 py-0.5 bg-surface-600 rounded text-[9px]">Alt</kbd> + <kbd class="px-1 py-0.5 bg-surface-600 rounded text-[9px]">V</kbd> <span class="text-surface-500">(hold)</span></span>
							</div>
							<div class="flex justify-between">
								<span class="text-surface-300">Send message</span>
								<span class="text-surface-400"><kbd class="px-1 py-0.5 bg-surface-600 rounded text-[9px]">Enter</kbd></span>
							</div>
							<div class="flex justify-between">
								<span class="text-surface-300">New line</span>
								<span class="text-surface-400"><kbd class="px-1 py-0.5 bg-surface-600 rounded text-[9px]">Shift</kbd> + <kbd class="px-1 py-0.5 bg-surface-600 rounded text-[9px]">Enter</kbd></span>
							</div>
							<div class="flex justify-between">
								<span class="text-surface-300">Stop streaming</span>
								<span class="text-surface-400"><kbd class="px-1 py-0.5 bg-surface-600 rounded text-[9px]">Esc</kbd></span>
							</div>
							<div class="flex justify-between">
								<span class="text-surface-300">Audio device</span>
								<span class="text-surface-400"><span class="text-surface-500">Right-click</span> mic</span>
							</div>
						</div>
					</div>
				{/if}
			...
			<button
				onclick={() => autoSpeak.update((v) => !v)}
				class="text-[9px] px-1 rounded transition-colors
					{$autoSpeak ? 'text-hecate-400' : 'text-surface-500 hover:text-surface-300'}"
		...
		<div bind:this={messagesEl} class="flex-1 overflow-y-auto p-4 space-y-3">
			{#each chatMessages as msg}
				...
				{:else if msg.role === 'assistant'}
					<div class="flex justify-start group/msg">
						<div
							class="max-w-[85%] rounded-lg px-3 py-2 text-[11px]
						...
						>
							...
							{#if !msg.content.startsWith('Error:')}
								<button
									onclick={() => speakMessage(stripVisionFences(msg.content))}
									disabled={$isSpeaking}
									class="mt-1 text-surface-500 hover:text-hecate-400 transition-colors opacity-0 group-hover/msg:opacity-100
										{$isSpeaking ? 'cursor-wait' : ''}"
									title="Read aloud"
								>
									<svg class="w-3 h-3" viewBox="0 0 24 24" fill="currentColor">
		...
		<div class="border-t border-surface-600 p-3 shrink-0">
			<div class="flex gap-2">
				...
				></textarea>
				<VoiceControls {ctx} onTranscript={handleTranscript} />
				<button
