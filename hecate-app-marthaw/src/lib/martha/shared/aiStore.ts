...
export const autoSpeak = writable(false);
export const voiceProvider = writable('groq');
export const voiceVoice = writable('tara');
export const isRecording = writable(false);
export const isSpeaking = writable(false);
export const selectedAudioDeviceId = writable<string | null>(
	localStorage.getItem('hecate-audio-device-id')
);

export function setAudioDevice(deviceId: string | null): void {
	selectedAudioDeviceId.set(deviceId);
	if (deviceId) {
		localStorage.setItem('hecate-audio-device-id', deviceId);
	} else {
		localStorage.removeItem('hecate-audio-device-id');
	}
}
...
function classifyTier(m: LLMModel): ModelTier {
	...
	// Balanced: good general-purpose (Groq models are fast + cheap)
	if (/llama-3\.3-70b|llama-3\.1-70b/.test(name)) return 'balanced';
...
function scoreModel(m: LLMModel, affinity: 'code' | 'general' | 'creative'): number {
	...
	// Groq preference: fast inference, cost-effective — strong bonus to make it default
	if (m.provider === 'groq') score += 30;

	// Penalize expensive providers (use only as fallback)
	if (m.provider === 'openai' || m.provider === 'anthropic' || m.provider === 'google') score -= 20;
