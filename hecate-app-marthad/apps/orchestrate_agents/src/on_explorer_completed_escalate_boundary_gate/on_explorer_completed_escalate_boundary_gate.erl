%%% @doc Process manager: on explorer_completed_v1, auto-escalate to boundary_gate.
%%% Subscribes to explorer_completed_v1 from orchestration_store.
-module(on_explorer_completed_escalate_boundary_gate).
-behaviour(gen_server).
-export([start_link/0]).
-export([init/1, handle_call/3, handle_cast/2, handle_info/2, terminate/2]).

-define(EVENT_TYPE, <<"explorer_completed_v1">>).
-define(SUB_NAME, <<"on_explorer_completed_escalate_boundary_gate">>).
...
process_event(RawEvent) ->
    ...
    case SessionId of
        ...
        _ ->
            ...
            case escalate_boundary_gate_v1:new(CmdParams) of
                {ok, Cmd} ->
                    case maybe_escalate_boundary_gate:dispatch(Cmd) of
                        {ok, _, _} ->
                            logger:info("[~s] escalated ~s to boundary_gate",
                                       [?MODULE, SessionId]);
                        {error, Reason} ->
