%%% @doc LLM runner PM: on architect initiated, run LLM and complete/fail.
%%% Subscribes to architect_initiated_v1 from orchestration_store.
%%% Loads role spec, builds prompt, calls chat_to_llm, parses notation,
%%% then dispatches complete or fail command.
-module(on_architect_initiated_run_architect_llm).
-behaviour(gen_server).
-export([start_link/0]).
-export([init/1, handle_call/3, handle_cast/2, handle_info/2, terminate/2]).

-define(EVENT_TYPE, <<"architect_initiated_v1">>).
-define(SUB_NAME, <<"on_architect_initiated_run_architect_llm">>).
-define(STORE_ID, orchestration_store).
-define(ROLE, <<"architect">>).
...
handle_llm_success(SessionId, Response) ->
    ...
    case complete_architect_v1:new(CmdParams) of
        {ok, Cmd} ->
            case maybe_complete_architect:dispatch(Cmd) of
                {ok, _, _} ->
                    logger:info("[~s] completed architect session ~s", [?MODULE, SessionId]);
                {error, Reason} ->
...
handle_llm_failure(SessionId, Reason) ->
    ...
    case fail_architect_v1:new(CmdParams) of
        {ok, Cmd} ->
            case maybe_fail_architect:dispatch(Cmd) of
                {ok, _, _} ->
                    logger:warning("[~s] architect session ~s failed: ~s",
