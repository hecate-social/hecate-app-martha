%%% @doc Process manager: on venture_initiated, initiate coordinator.
%%% Subscribes to venture_initiated_v1 from martha_store.
-module(on_venture_initiated_initiate_coordinator).
-behaviour(gen_server).
-export([start_link/0]).
-export([init/1, handle_call/3, handle_cast/2, handle_info/2, terminate/2]).

-define(EVENT_TYPE, <<"venture_initiated_v1">>).
-define(SUB_NAME, <<"on_venture_initiated_initiate_coordinator">>).
