...
handle_llm_success(SessionId, Response) ->
    ...
    case complete_explorer_v1:new(CmdParams) of
        {ok, Cmd} ->
            case maybe_complete_explorer:dispatch(Cmd) of
                {ok, _, _} ->
                    logger:info("[~s] completed explorer session ~s", [?MODULE, SessionId]);
                {error, Reason} ->
...
handle_llm_failure(SessionId, Reason) ->
    ...
    case fail_explorer_v1:new(CmdParams) of
        {ok, Cmd} ->
            case maybe_fail_explorer:dispatch(Cmd) of
                {ok, _, _} ->
                    logger:warning("[~s] explorer session ~s failed: ~s",
