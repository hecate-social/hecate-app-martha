%%% @doc LLM runner PM: on svelte_coder initiated, run LLM and complete/fail.
%%% Subscribes to svelte_coder_initiated_v1 from orchestration_store.
%%% Loads role spec, builds prompt, calls chat_to_llm, parses notation,
%%% then dispatches complete or fail command.
-module(on_svelte_coder_initiated_run_svelte_coder_llm).
-behaviour(gen_server).
-export([start_link/0]).
-export([init/1, handle_call/3, handle_cast/2, handle_info/2, terminate/2]).

-define(EVENT_TYPE, <<"svelte_coder_initiated_v1">>).
-define(SUB_NAME, <<"on_svelte_coder_initiated_run_svelte_coder_llm">>).
-define(STORE_ID, orchestration_store).
-define(ROLE, <<"svelte_coder">>).
...
handle_llm_success(SessionId, Response) ->
    ...
    case complete_svelte_coder_v1:new(CmdParams) of
        {ok, Cmd} ->
            case maybe_complete_svelte_coder:dispatch(Cmd) of
                {ok, _, _} ->
                    logger:info("[~s] completed svelte_coder session ~s", [?MODULE, SessionId]);
                {error, Reason} ->
...
handle_llm_failure(SessionId, Reason) ->
    ...
    case fail_svelte_coder_v1:new(CmdParams) of
        {ok, Cmd} ->
            case maybe_fail_svelte_coder:dispatch(Cmd) of
                {ok, _, _} ->
                    logger:warning("[~s] svelte_coder session ~s failed: ~s",
