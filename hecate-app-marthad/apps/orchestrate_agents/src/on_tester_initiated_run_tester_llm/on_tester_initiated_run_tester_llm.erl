%%% @doc LLM runner PM: on tester initiated, run LLM and complete/fail.
%%% Subscribes to tester_initiated_v1 from orchestration_store.
%%% Loads role spec, builds prompt, calls chat_to_llm, parses notation,
%%% then dispatches complete or fail command.
-module(on_tester_initiated_run_tester_llm).
-behaviour(gen_server).
-export([start_link/0]).
-export([init/1, handle_call/3, handle_cast/2, handle_info/2, terminate/2]).

-define(EVENT_TYPE, <<"tester_initiated_v1">>).
-define(SUB_NAME, <<"on_tester_initiated_run_tester_llm">>).
...
handle_llm_success(SessionId, Response) ->
    ...
    case complete_tester_v1:new(CmdParams) of
        {ok, Cmd} ->
            case maybe_complete_tester:dispatch(Cmd) of
                {ok, _, _} ->
                    logger:info("[~s] completed tester session ~s", [?MODULE, SessionId]);
                {error, Reason} ->
...
handle_llm_failure(SessionId, Reason) ->
    ...
    case fail_tester_v1:new(CmdParams) of
        {ok, Cmd} ->
            case maybe_fail_tester:dispatch(Cmd) of
                {ok, _, _} ->
                    logger:warning("[~s] tester session ~s failed: ~s",
