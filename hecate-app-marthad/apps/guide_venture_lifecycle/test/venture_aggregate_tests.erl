...
exec_non_initiate_on_fresh() ->
    Cmd = #{command_type => <<"refine_vision_v1">>,
            <<"venture_id">> => <<"v-test-1">>},
...
exec_submit_vision_ok() ->
    Cmd = #{command_type => <<"submit_vision_v1">>,
            <<"venture_id">> => <<"v-test-1">>},
    {ok, [Event]} = venture_aggregate:execute(initiated_state(), Cmd),
...
exec_start_discovery_ok() ->
    Cmd = #{command_type => <<"start_discovery_v1">>,
            <<"venture_id">> => <<"v-test-1">>},
    {ok, [Event]} = venture_aggregate:execute(initiated_state(), Cmd),
...
exec_identify_division_ok() ->
    Cmd = #{command_type => <<"identify_division_v1">>,
            <<"venture_id">> => <<"v-test-1">>,
...
exec_identify_division_duplicate() ->
    ...
    EvtMap = #{event_type => <<"division_identified_v1">>,
               ...
               <<"division_id">> => <<"div-1">>,
               <<"context_name">> => <<"auth_division">>},
    State1 = venture_aggregate:apply(State0, EvtMap),
...
exec_pause_discovery_ok() ->
    Cmd = #{command_type => <<"pause_discovery_v1">>,
            <<"venture_id">> => <<"v-test-1">>,
...
exec_resume_discovery_ok() ->
    Cmd = #{command_type => <<"resume_discovery_v1">>,
            <<"venture_id">> => <<"v-test-1">>},
...
exec_complete_discovery_ok() ->
    Cmd = #{command_type => <<"complete_discovery_v1">>,
            <<"venture_id">> => <<"v-test-1">>},
    {ok, [Event]} = venture_aggregate:execute(discovering_state(), Cmd),
...
exec_archive_ok() ->
    Cmd = #{command_type => <<"archive_venture_v1">>,
            <<"venture_id">> => <<"v-test-1">>,
...
apply_vision_refined() ->
    Event = #{event_type => <<"vision_refined_v1">>,
              ...
              <<"skills">> => [<<"go">>],
              <<"context_map">> => #{<<"x">> => <<"y">>}},
    State = venture_aggregate:apply(initiated_state(), Event),
...
apply_division_identified() ->
    ...
    State = venture_aggregate:apply(discovering_state(), Event),
...
apply_discovery_resumed() ->
    Event = #{event_type => <<"discovery_resumed_v1">>,
              <<"venture_id">> => <<"v-test-1">>},
    State = venture_aggregate:apply(paused_state(), Event),
    ?assert(State#venture_state.status band ?VL_DISCOVERING =/= 0),
    ?assert(State#venture_state.status band ?VL_DISCOVERY_PAUSED =:= 0),
...
apply_unknown_event() ->
    ...
    State = initiated_state(),
    ?assertEqual(State, venture_aggregate:apply(State, Event)).
...
apply_binary_keys() ->
    Event = #{event_type => <<"venture_initiated_v1">>,
              ...
              <<"name">> => <<"Binary Keys">>,
              <<"initiated_at">> => 1000},
    State = venture_aggregate:apply(fresh(), Event),
...
full_lifecycle() ->
    ...
    {ok, [E7]} = venture_aggregate:execute(S6,
        #{command_type => <<"resume_discovery_v1">>,
          <<"venture_id">> => <<"v-full">>}),
    ...
    {ok, [E8]} = venture_aggregate:execute(S7,
        #{command_type => <<"complete_discovery_v1">>,
          <<"venture_id">> => <<"v-full">>}),
