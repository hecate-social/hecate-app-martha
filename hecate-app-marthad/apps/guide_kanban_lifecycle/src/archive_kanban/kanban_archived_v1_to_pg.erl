%%% @doc Emitter: kanban_archived_v1 -> pg (internal pub/sub)
%%% Subscribes to martha_store via evoq, broadcasts to pg group.
-module(kanban_archived_v1_to_pg).
-behaviour(gen_server).
-export([start_link/0]).
-export([init/1, handle_call/3, handle_cast/2, handle_info/2, terminate/2]).

-define(EVENT_TYPE, <<"kanban_archived_v1">>).
-define(PG_GROUP, kanban_archived_v1).
-define(SUB_NAME, <<"kanban_archived_v1_to_pg">>).
