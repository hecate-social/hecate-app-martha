...
validate(#design_event_v1{division_id = D}) when not is_binary(D); byte_size(D) =:= 0 ->
    {error, invalid_division_id};
validate(#design_event_v1{event_name = N}) when not is_binary(N); byte_size(N) =:= 0 ->
...
-spec get_fields(design_event_v1()) -> list().
