...
new(#{division_id := DivisionId, suite_id := SuiteId, suite_name := SuiteName}) ->
    #test_suite_run_v1{
        ...
        run_at = erlang:system_time(millisecond)
    }.
    ...
    case {DivisionId, SuiteId} of
        ...
        _ ->
            {ok, #test_suite_run_v1{
                division_id = DivisionId,
                suite_id = SuiteId,
                suite_name = get_value(suite_name, Map, <<>>),
                run_at = get_value(run_at, Map, erlang:system_time(millisecond))
            }}
    end.

%% Accessors
-spec get_division_id(test_suite_run_v1()) -> binary().
get_division_id(#test_suite_run_v1{division_id = V}) -> V.
-spec get_suite_id(test_suite_run_v1()) -> binary().
get_suite_id(#test_suite_run_v1{suite_id = V}) -> V.
-spec get_suite_name(test_suite_run_v1()) -> binary().
get_suite_name(#test_suite_run_v1{suite_name = V}) -> V.
-spec get_run_at(test_suite_run_v1()) -> integer().
