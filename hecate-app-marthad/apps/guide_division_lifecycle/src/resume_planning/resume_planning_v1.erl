%%% @doc resume_planning_v1 command
...
-export_type([resume_planning_v1/0]).
-opaque resume_planning_v1() :: #resume_planning_v1{}.
...
-spec validate(resume_planning_v1()) -> {ok, resume_planning_v1()} | {error, term()}.
validate(#resume_planning_v1{division_id = D}) when not is_binary(D); byte_size(D) =:= 0 ->
    {error, invalid_division_id};
validate(#resume_planning_v1{} = Cmd) ->
    {ok, Cmd}.
...
from_map(Map) ->
    ...
    case DivisionId of
        ...
        _ -> {ok, #resume_planning_v1{division_id = DivisionId}}
    end.

%% Accessors
-spec get_division_id(resume_planning_v1()) -> binary().
get_division_id(#resume_planning_v1{division_id = V}) -> V.
