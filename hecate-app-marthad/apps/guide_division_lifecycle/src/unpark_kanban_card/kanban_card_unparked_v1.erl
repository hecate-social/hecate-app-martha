...
get_division_id(#kanban_card_unparked_v1{division_id = V}) -> V.
get_card_id(#kanban_card_unparked_v1{card_id = V}) -> V.
...
new(Params) ->
    #kanban_card_unparked_v1{
        ...
        unparked_at = erlang:system_time(millisecond)
    }.
...
-spec from_map(map()) -> {ok, kanban_card_unparked_v1()} | {error, term()}.
from_map(Map) ->
    {ok, #kanban_card_unparked_v1{
        division_id = get_value(division_id, Map),
        card_id     = get_value(card_id, Map),
        unparked_at = get_value(unparked_at, Map, erlang:system_time(millisecond))
    }}.
