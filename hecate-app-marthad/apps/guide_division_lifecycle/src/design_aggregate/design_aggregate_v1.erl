...
validate(#design_aggregate_v1{division_id = D}) when not is_binary(D); byte_size(D) =:= 0 ->
    {error, invalid_division_id};
validate(#design_aggregate_v1{aggregate_name = N}) when not is_binary(N); byte_size(N) =:= 0 ->
...
from_map(Map) ->
    ...
    AggName = get_value(aggregate_name, Map),
    case {DivisionId, AggName} of
...
-spec get_fields(design_aggregate_v1()) -> list().
