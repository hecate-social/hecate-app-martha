...
validate(#plan_dependency_v1{division_id = D}) when not is_binary(D); byte_size(D) =:= 0 ->
...
validate(#plan_dependency_v1{} = Cmd) ->
    {ok, Cmd}.
...
from_map(Map) ->
    ...
    case {DivisionId, FromDesk, ToDesk} of
        ...
        _ ->
            DepId = get_value(dependency_id, Map, generate_id()),
            {ok, #plan_dependency_v1{
                division_id = DivisionId,
                dependency_id = DepId,
                from_desk = FromDesk,
